...
async def test_interval(
    ...
    async with engine.begin() as conn:
        await conn.run_sync(base.metadata.create_all)
    async with db.begin() as sess:
        sess.add(TestModel(duration=timedelta(hours=2, minutes=30)))
    ...
    # timedelta values must serialize (Encoder previously raised TypeError -> 500).
    url = app[admin].router["test_get_one"].url_for()
    async with admin_client.get(url, params={"id": 1}, headers=h) as resp:
        assert resp.status == 200
        assert await resp.json() == {"data": {"id": "1", "data": {"id": 1, "duration": "2:30:00"}}}
