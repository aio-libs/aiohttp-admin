...
from aiohttp_admin.backends.sa_utils import table_to_trafaret
...
def test_keys(table):
    names = sorted(['id', 'title', 'body', 'views', 'average_note', 'pictures',
                    'published_at', 'tags'])
    traf = table_to_trafaret(table, 'id', skip_pk=False)
...
def test_skip_pk(table):
    names = sorted(['title', 'body', 'views', 'average_note', 'pictures',
                    'published_at', 'tags'])
    traf = table_to_trafaret(table, 'id', skip_pk=True)
    result_names = sorted([key.name for key in traf.keys])
    assert result_names == names
